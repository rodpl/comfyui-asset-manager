...
- **Styling**: Standard CSS only (must use CSS classes provided by ComfyUI, no custom UI frameworks allowed)
...
- **Styling Constraints**: **CRITICAL** - Cannot use custom UI frameworks (no PrimeVue, Material-UI, etc.)

### CSS and Styling Rules ⚠️ CRITICAL CONSTRAINT
- **Framework Restriction**: **ABSOLUTELY NO** custom UI frameworks (PrimeVue, Material-UI, Ant Design, etc.)
- **Available CSS**: Must use only CSS classes provided by ComfyUI frontend
- **ComfyUI Dependencies**: Can reference CSS from ComfyUI's dependencies:
  - Reference: https://github.com/Comfy-Org/ComfyUI_frontend/blob/main/package.json
  - Available frameworks: TailwindCSS, PrimeVue (but only classes already loaded by ComfyUI)
- **Custom CSS**: Can define own CSS files but must not conflict with ComfyUI's styling
- **Plugin Nature**: Extension acts as plugin within ComfyUI's environment - cannot load additional CSS frameworks

### Styling Strategy
1. **Inspect ComfyUI**: Use browser dev tools to identify available CSS classes
2. **TailwindCSS**: Use utility classes already loaded by ComfyUI
3. **PrimeVue Classes**: Use component classes already available in ComfyUI
4. **Custom CSS**: Write minimal custom CSS for unique styling needs
5. **No Bundling**: Never bundle additional CSS frameworks in the extension
...
4. **CSS Restrictions**: **NEVER** import custom UI frameworks - use only ComfyUI-provided CSS classes
5. **Styling Strategy**: Reference ComfyUI frontend dependencies or create custom CSS that doesn't conflict
...
- **CSS Framework Error**: If UI looks broken, verify no custom frameworks were imported
- **Missing Styles**: Use browser dev tools to find available ComfyUI CSS classes
